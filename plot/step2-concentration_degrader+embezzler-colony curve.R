...
folderpath <- paste(as.character(path1), "/","data11-concentration curve/", sep="")
setwd(folderpath) 

datacon_de<-read.csv("plot-Concenlist_diameter-average-degrader.csv",header = T)
datacon_em<-read.csv("plot-Concenlist_diameter-average-embezzler.csv",header = T)

datacon_de$strain <- c("de")
datacon_em$strain <- c("em")
datacon<-rbind(datacon_de,datacon_em)
write.csv(datacon,"plot-Concenlist_diameter-average-all.csv", row.names=FALSE)
...
for(m in 1:length(mass_list)){
  ...
  par(mar=c(5,7,2,2))
  p <-ggplot(data=df1, aes(x=radius, y=concentration, colour = as.factor(substrate),fill=as.factor(substrate),linetype=as.factor(strain))) + 
    geom_line(size=1)+
  ...
  dev.off()
  mass_name <- paste("Concenlist_diameter-all-", mass_list[[m]],".pdf",sep="")
...
###ratio
datacon_de_1<-read.csv("Concenlist_diameter-degrader.csv",header = T)
datacon_em_1<-read.csv("Concenlist_diameter-embezzler.csv",header = T)
datacon_de_2<-datacon_de_1[,5:(ncol(datacon_de_1))]
datacon_em_2<-datacon_em_1[,5:(ncol(datacon_em_1))]
datacon_merge <- merge(datacon_de_2,datacon_em_2,by=c("radius","substrate","replicate"),all=T)

names(datacon_merge)[4:ncol(datacon_merge)] <- c("Sout_de","Sin_de","Iout_de","Iin_de","Pout_de","Pin_de","mu_de","cell_num_de","Stotal_de","Itotal_de","Ptotal_de","Sout_em","Sin_em","Iout_em","Iin_em","Pout_em","Pin_em","mu_em","cell_num_em","Stotal_em","Itotal_em","Ptotal_em")
...
write.csv(datacon_merge_ratio,"Concenratio_list_diameter.csv", row.names=FALSE)
...
for(m in 1:length(ratio_list)){
  df_ratio<-summarySE(datacon_merge_ratio,measurevar=ratio_list[[m]],groupvars=c("radius","substrate"))
  
  df1 <-df_ratio[which(df_ratio$radius>=8),]
  par(mar=c(5,7,2,2))
  p <-ggplot(data=df1, aes(x=radius, y=df1[,ratio_list[[m]]], colour = as.factor(substrate),fill=as.factor(substrate))) + 
    geom_line(size=1)+
    geom_ribbon(aes(ymin = df1[,ratio_list[[m]]] - sd,
                    ymax = df1[,ratio_list[[m]]] + sd,fill=as.factor(substrate)),
                alpha=0.2,linetype = 0)+
    scale_fill_manual(values = c('#8cadeb',"#de9a7e",'#50bdaf','#cf99d3'),
                      limits = c("5","10","15","20"))+
    scale_color_manual(values = c('#8cadeb',"#de9a7e",'#50bdaf','#cf99d3'),
                       limits = c("5","10","15","20"))+
    labs(x="Distance from center (R)",y="degrader/consumer")+
    theme(panel.grid = element_blank(), panel.background = element_rect(color = 'black', fill = 'transparent'))+
    theme(axis.text= element_text(size=14,color="black"),
          axis.title.x=element_text(size=16),axis.title.y=element_text(size=16))#+
  #theme(legend.position = 'none')
  dev.off()
  mass_name <- paste("Concenratio_list_diameter-", ratio_list[[m]],".pdf",sep="")
  ggsave(mass_name,height=6,width=8)
  dev.new()
  names(df_ratio)[4:5] <-c("ratio_ave","ratio_sd")
  df_ratio_ave <- rbind(df_ratio_ave,cbind(df_ratio[1:5],mass=ratio_list[[m]]))
}
write.csv(df_ratio_ave,"plot-Concenratio_list_diameter.csv", row.names=FALSE)
