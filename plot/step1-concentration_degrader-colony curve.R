...
setwd(folderpath) 
for(k in 1:3){#k:replicates
##position
datap_de_name <- paste("data2p",k,".csv",sep="")
...
##mass concentration
datacon_de_name <- paste("dataD",k,".txt",sep="")
...
con_name <- paste("Concenlist_diameter-degrader", ".csv",sep="")
folderpath2 <- paste(as.character(path1), "/","data11-concentration curve/",sep="")
setwd(folderpath2) 
write.csv(Alldata,con_name, row.names=FALSE)

Alldata<-read.csv("Concenlist_diameter-degrader.csv",header = T)
...
dev.off()
mass_name <- paste("Concenlist_diameter-degrader-", mass_list[[m]],".pdf",sep="")
...
write.csv(df_all,"plot-Concenlist_diameter-average-degrader.csv", row.names=FALSE)